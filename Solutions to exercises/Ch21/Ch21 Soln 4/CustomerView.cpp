...
void CCustomerView::DoDataExchange(CDataExchange* pDX)
{
  ...
  DDX_Control(pDX, IDC_SHIPPERS, m_ShippersCtrl);
}
...
void CCustomerView::OnInitialUpdate()
{
  ...
  CShippersSet* pShippersSet = &static_cast<CDBSimpleUpdateDoc*>(GetDocument())->m_Shippers;
...
BEGIN_MESSAGE_MAP(CCustomerView, CRecordView)
  ...
  ON_LBN_SELCHANGE(IDC_SHIPPERS, &CCustomerView::OnSelchangeShippers)
END_MESSAGE_MAP()
...
void CCustomerView::OnCancel()
{
  ...
  // Reset Shippers  list box
  m_ShippersCtrl.SetTopIndex(0);                    // Move to first item
  m_ShipVia = NO_SHIPPERS_ID;                       // Reset ID to no selection
...
{
  m_ShipVia = static_cast<long>(m_ShippersCtrl.GetItemData(m_ShippersCtrl.GetCurSel()));
}
